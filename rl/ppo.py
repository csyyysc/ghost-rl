...
class ActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_dim=256):
        ...
        # Image-based processing (CarRacing has 96x96x3 input)
    ...
    def _get_conv_output_size(self, input_shape, input_height, input_width):
        """Calculate the output size of convolutional layers"""

        dummy_input = torch.zeros(
            1, input_shape, input_height, input_width)
...
class PPOAgent:
    ...
    def __init__(self, env, lr=3e-4, gamma=0.99, clip_ratio=0.2,
        ...
        self.actor_critic = ActorCritic(
            env.observation_space, env.action_space).to(self.device)
...
def train():
    ...
    print(f"Training PPO on Pendulum-v1")
...
def simulate(model_path):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            with torch.no_grad():
                action, _, _, _ = agent.actor_critic.get_action_and_value(
                    state)
